...
При очистке поля должны генерироваться событие `input` и `change` (по идее оно должно генерироваться само при уходе фокуса с инпута, но фокус может уйти раньше, чем поменялись знаечения в инпуте) так, что если кто-то подписался на события, то узнает о том, что поле изменило значение. 
...
Некоторые возможности jQuery UI теперь реализуются на HTML5/CSS3 (selectable, календарик, прогрессбар, spinner, slider). Для создания автокомплитов есть аттрибут `datalist`.
...
- HTML5 Audio API — стандарт, который позволяет обрабатывать и генерировать звук яваскриптом и строить цепочки эффектов, обрабатывающих звук в реальном времени. Примеры: http://webaudioapi.com/samples/
